...
const GetBalanceViewForUpdate = `-- name: GetBalanceViewForUpdate :one
SELECT id, balance, available, pending, version, created_at, updated_at
FROM balance_views
WHERE id = $1
FOR UPDATE
...
func (q *Queries) GetBalanceViewForUpdate(ctx context.Context, id uuid.UUID) (*BalanceView, error) {
	row := q.db.QueryRow(ctx, GetBalanceViewForUpdate, id)
	var i BalanceView
	err := row.Scan(
		&i.ID,
		&i.Balance,
